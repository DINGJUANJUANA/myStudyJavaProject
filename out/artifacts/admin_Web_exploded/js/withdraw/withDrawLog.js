/**
 * Created by chenchunchuan on 2017/10/15.
 */
var vm;
...
$(function () {
    ...
    vm = new Vue({
        ...
        methods: {
            ...
            download: function (event) {
                var url = '../withdrawLog/downloadExcelList?' + $("#withDraw").serialize();
                // console.log(url);
                window.open(url);
            }
        }
    ...
    $("#jqGrid").jqGrid({
        url: '../withdrawLog/list',
        datatype: "json",
        colModel: [
            {label: '提现流水ID', name: 'id', width: 40},
            {label: '客户作业账户编码', name: 'accountOrgNo', width: 80},
            {label: '客户身份证', name: 'idno', width: 80},
            {label: '提现订单号', name: 'orderId', width: 80},
            {label: '交易对方账户', name: 'bankCardNo', width: 80},
            {label: '提现类别', name: 'method', width: 80},
            {label: '提现金额', name: 'amount', width: 40},
            {label: '提现提交时间', name: 'startDate', width: 80},
            {label: '提现结束时间', name: 'endDate', width: 80},
            {label: '提现状态', name: 'status', width: 40},
...
function formatFun(cellvalue, options, rowObjec) {
    if(hasPermission('balance:withdrawLog:detail')) {
...
function zqgx(id) {
    vm.showList = false;
    $.get("../withdrawLog/info/" + id, function (r) {
        vm.withDrawDetail = r.withDrawDetail;
    });

    $.jgrid.gridUnload('jqGridAccountIn');
    $("#jqGridAccountIn").jqGrid({
        url: "../withdrawLog/accountIn/" + id,
        datatype: "json",
        colModel: [
