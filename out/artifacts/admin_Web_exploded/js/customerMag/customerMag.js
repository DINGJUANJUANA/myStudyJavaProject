...
function loanReload() {
    // var page = $("#jqGrid2").jqGrid('getGridParam', 'page');
    $("#jqGridloanLog").jqGrid('setGridParam',{
        page : 1,
        postData : serializeObject($("#form-6"))
    }).trigger("reloadGrid");
}
...
$(function(){
    ...
    $("#jqGrid").jqGrid({
        ...
        colModel : [
            ...
            {label: '用户账户编号', name: 'accountOrgId', width: 50, hidden:true},
            {label: '存管账户编码', name: 'accountId', width: 110},
            {label: '用户姓名/企业名称', name: 'realName', width: 110},
            {label: '注册手机号', name: 'mobile', width: 110},
            {label: '创建时间', name: 'createTime', width: 140},
            {label: '用户角色', name: 'userType', width: 140},
            {label: '账户总余额', name: 'totalAmount', width: 110},

            {label: '开启自动投标', name: 'openFlag', width: 140,formatter:function (value, options, row) {
                    return value == 1 ?
                        '<span>开启</span>':
                        '<span>关闭</span>';
                }},
...
function showInfo(id) {
    ...
    $("#loanAccountId").val(customerAccount.accountId);
    ...
    var userTypeCode=customerAccount.userTypeCode;
    if(userTypeCode==1||userTypeCode==3){
        $(".invest").hide();
        $(".loan").show();
    }else{
        $(".loan").hide();
    ...
    $("#jqGrid1").jqGrid({
        ...
        postData : serializeObject($("#form-1")),
        colModel : [
            /*{label: '收支流水ID', name: 'orderNo', width: 200, key: true},*/
    ...
    $("#jqGridLoanLog").jqGrid({
        url : "../customerMag/findLoanLog",
        datatype : "json",
        postData : serializeObject($("#form-6")),
        colModel : [
            {label: '创建时间', name: 'createTime',width:100},
            {label: '标的编号', name: 'borrowNo', width: 100, key: true},
            {label: '资金端标的编号', name: 'borrowNo', width: 150},
            {label: '借款金额（元）', name: 'contractAmount', width: 150},
            {label: '年化利率（%）', name: 'annualizedRate', width: 150},
            {label: '还款方式', name: 'returnType', width: 150},
            {label: '借款期限', name: 'periodUnit', width: 150},
            {label: '标的期数', name: 'periodLength', width: 150},
            {label: '标的状态', name: 'status', width: 150},

            {label: '合同预览', name: 'investAmount', width: 150},
        ...
        gridComplete: function () {
            //隐藏grid底部滚动条
            $("#jqGridLoanLog").closest(".ui-jqgrid-bdiv").css({"overflow-x": "hidden"});
        }
    });
