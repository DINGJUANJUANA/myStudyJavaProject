...
$(function () {
    ...
    $("#modal-sub").on({
        click: function () {
            if (!isNumber(vm.fromRaise.periodLength) || parseInt(vm.fromRaise.periodLength) < 0 || parseInt(vm.fromRaise.periodLength) % 1 > 0) {
                alert("手续费必须大于0");
                return;
            }
            if (isBlank(vm.fromRaise.date)) {
                alert("请选择时间");
                return;
            }

            var param = {
                "startTime": vm.fromRaise.date,
                "raisePeriod": vm.fromRaise.periodLength,
                "borrowNo": vm.fromRaise.borrowNo
            };
            console.log(param);
...
function formatFun(cellvalue, options, rowObjec) {
    var res = '<p class="btn btn-success btn-sm" onclick="detail(\'' + cellvalue + '\')">查看详情</p>&nbsp' +
        '<p class="btn btn-success btn-sm" onclick="publish(\'' + cellvalue + '\')">发布</p>&nbsp';
    return res;
}
...
function publish(borrowNo) {
    var borrowno = null;
    if(borrowNo != null){
        borrowno = borrowNo
    }else {
        borrowno = vm.fromBorrow.borrowNo
    }
    confirm('请问确定要发布该标的吗？', function () {
        $.get("../borrowApprove/publish?borrowNo=" + borrowno, function (r) {
            if (r.code == 0) {
                alert("发布成功");
                vm.query();
            } else {
